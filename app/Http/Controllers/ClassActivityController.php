        ...
        try {
            ...
            foreach($datas as $data){
                ...
                if ($diffInMinutes == 60) {
                    ...
                    DB::connection('clykk_lifestyle')->statement("INSERT INTO invites (name, email, mobile, `type`, class_date, class_time, class_link, created_at, updated_at, class_name, sent, user_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [$data->getUser->first_name.' '.$data->getUser->last_name, $data->getUser->email, $data->getUser->mobile, 'notification', $data->getSession->class_date, $data->getSession->class_time, $data->getSession->join_url, date('Y-m-d, H:i:s'), date('Y-m-d, H:i:s'), $data->getSession->getPackage->package_name, 0, $data->getUser->id]);
