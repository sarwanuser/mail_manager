            ...
            // if(@$token_status['status'] == '200'){ 
                ...
                if($request->filter != ''){
                    ...
                    if($request->status == 'all' && $request->city != 'All'){
                        $datas = SubscriptionDatas::select('cart_id as cartID','created_at as createdAt','id','resched_count as reschedCount','service_date as serviceDate', 'service_time as serviceTime', 'status','updated_at as updatedAt','city','state','sub_category_id','subCategoryName','category_id','categoryName')->with('getAddressDetails')->whereBetween('service_date', [$from, $to])->where('city',$request->city)->distinct('id')->orderBy('id', 'DESC')->paginate($request->per_page)->toArray();

                    }elseif($request->status != 'all' && $request->city == 'All'){
                        $datas = SubscriptionDatas::select('cart_id as cartID','created_at as createdAt','id','resched_count as reschedCount','service_date as serviceDate', 'service_time as serviceTime', 'status','updated_at as updatedAt','city','state','sub_category_id','subCategoryName','category_id','categoryName')->with('getAddressDetails')->whereBetween('service_date', [$from, $to])->where('status',$request->status)->distinct('id')->orderBy('id', 'DESC')->paginate($request->per_page)->toArray();

                    }elseif($request->status == 'all' && $request->city == 'All'){
                        $datas = SubscriptionDatas::select('cart_id as cartID','created_at as createdAt','id','resched_count as reschedCount','service_date as serviceDate', 'service_time as serviceTime', 'status','updated_at as updatedAt','city','state','sub_category_id','subCategoryName','category_id','categoryName')->with('getAddressDetails')->whereBetween('service_date', [$from, $to])->distinct('id')->orderBy('id', 'DESC')->paginate($request->per_page)->toArray();

                    }else{
                        $datas = SubscriptionDatas::select('cart_id as cartID','created_at as createdAt','id','resched_count as reschedCount','service_date as serviceDate', 'service_time as serviceTime', 'status','updated_at as updatedAt','city','state','sub_category_id','subCategoryName','category_id','categoryName')->with('getAddressDetails')->whereBetween('service_date', [$from, $to])->where('city',$request->city)->where('status',$request->status)->distinct('id')->orderBy('id', 'DESC')->paginate($request->per_page)->toArray();
